...
process_t=[list(map(int, pt_tmp.iloc[i])) for i in range(num_job)]
# print(process_t)
machine_seq=[list(map(int,ms_tmp.iloc[i])) for i in range(num_job)]
# print(machine_seq)
...
num_iteration=int(input('Please input number of iteration: ') or 250) # default value is 2000
...
for n in range(num_iteration):
    ...
    for m in range(population_size*2):
        ...
        for i in total_chromosome[m]:
            gen_t=int(process_t[i][key_count[i]])
            gen_m=int(machine_seq[i][key_count[i]])
...
for i in sequence_best:
    ...
    start_time=str(datetime.timedelta(seconds=j_count[i]-process_t[i][key_count[i]])) # convert seconds to hours, minutes and seconds
